...
- Việc tạo bài giảng + câu hỏi được chia thành nhiều bước độc lập (`api/upload-document.ts` khởi tạo job, `api/upload-step.ts` xử lý từng bước): 1 bước bài giảng, rồi tới các bước **trích xuất riêng nội dung từng chương theo từng phần nhỏ** (`extractChapterChunk`, xem `planExtractionChunksPerChapter`), cuối cùng mới tới các lô sinh câu hỏi (đọc phần text đã trích xuất, không đọc lại tài liệu gốc). Việc trích xuất 1 chương dài trong 1 lần dễ vượt 60s dù chia lô câu hỏi nhỏ tới đâu — nên mỗi chương được trích xuất thành nhiều bước nối tiếp (mỗi bước tiếp nối ngay sau đoạn đã trích ở bước trước, dựa trên độ dài tài liệu đã biết), mỗi bước luôn đủ nhỏ để an toàn trong 60s. Trạng thái job được lưu tạm trong Redis (TTL 15 phút) để client gọi lặp lại `upload-step` cho tới khi xong; nếu 1 bước bị Vercel ngắt do hết 60s, client tự động thử lại đúng bước đó (job không bị hỏng vì tiến trình bị giết trước khi kịp lưu lỗi).
